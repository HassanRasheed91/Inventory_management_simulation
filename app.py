...
def run_simulation():
    ...
    try:
        simulation_status = "Training fresh RL models and running simulation... Please wait."
...
def run_rl_training():
    """Train RL models for every run"""
    print("Training fresh RL models...")
    ...
    try:
        ...
        # Train models
        training_results = train_and_evaluate_agents()
        
        print(f"✅ Successfully trained fresh models")
        print(f"Best model: {training_results['best_name']}")
        print(f"Best cost: {training_results['best_metrics']['total_cost']:.2f}")
        
        return {
            "status": "fresh_models_trained",
            "best_model": training_results['best_name'],
            "best_cost": training_results['best_metrics']['total_cost'],
            "ppo_cost": training_results['ppo_metrics']['total_cost'],
            "message": "Fresh models trained successfully",
            "training_results": training_results
        }
        
    except Exception as e:
        print(f"❌ Error training fresh models: {str(e)}")
        return {
            "status": "training_failed",
            "error": str(e),
            "message": "Failed to train fresh models"
        }
...
def create_final_report(results, report):
    ...
    final_report.append("- **Mission**: Minimize overall cost")
    final_report.append("- **Products**: 2 products with stochastic demand and lead times")
    final_report.append("- **Product 1 Demand**: D = {1,2,3,4} with probabilities {1/6, 1/3, 1/3, 1/6}")
    final_report.append("- **Product 2 Demand**: D = {2,3,4,5} with probabilities {1/8, 1/2, 1/4, 1/8}")
    final_report.append("- **Lead Times**: Product 1 ~ Uniform(0.5, 1), Product 2 ~ Uniform(0.2, 0.7)")
    final_report.append("- **Cost Structure**: K=10 (fixed), i=3 (variable), h=1 (holding), pi=7 (penalty)")
    final_report.append("- **Demand Inter-arrival**: Exponential random variable, lambda=0.1")
    final_report.append("")
