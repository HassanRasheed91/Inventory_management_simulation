...
class InventoryEnv(gym.Env):
    ...
    - i = 3 (variable ordering cost per unit)
    - h = 1 (holding cost per unit per day)
    - pi = 7 (penalty cost per unit of unmet demand)
    
    Product 1: Demand D = {1,2,3,4} with probabilities {1/6, 1/3, 1/3, 1/6}, Lead Time: U(0.5,1)
    Product 2: Demand D = {2,3,4,5} with probabilities {1/8, 1/2, 1/4, 1/8}, Lead Time: U(0.2,0.7)
    ...
    def __init__(self, max_stock: int = 50, max_order: int = 10):
        ...
        self.penalty_cost = 7.0  # pi = 7 per unit of unmet demand
        self.product1_demand_probs = [1/6, 1/3, 1/3, 1/6]  # For demands 1,2,3,4
        self.product1_demand_values = [1, 2, 3, 4]
        self.product2_demand_probs = [1/8, 1/2, 1/4, 1/8]  # For demands 5,4,3,2
        self.product2_demand_values = [5, 4, 3, 2]
    ...
    def step(self, action: Tuple[int, int]) -> Tuple[np.ndarray, float, bool, bool, Dict[str, Any]]:
        ...
        base_reward = -daily_cost
        
        # Service level penalty
        service_penalty = 0
        if shortage1 > 0:
            service_penalty += shortage1 * 7.0 
        if shortage2 > 0:
            service_penalty += shortage2 * 10.0 
        ...
        # Product 2
        if 2 <= self.stock[1] <= 10:  
            efficiency_bonus += 2.0
            
        # Ordering efficiency
        ordering_efficiency = 0
        # Product 1: order when stock <= 1
        if action[0] > 0 and self.stock[0] <= 1:  
            ordering_efficiency += action[0] * 0.5 
        # Product 2: order when stock <= 2 
        if action[1] > 0 and self.stock[1] <= 2:  
            ordering_efficiency += action[1] * 0.5  # Reduced bonus
            
        # Over-ordering penalty
        over_order_penalty = 0
        # Product 1: strong penalty when stock > 4
        if action[0] > 0 and self.stock[0] > 4:  
            over_order_penalty += action[0] * 5.0  # Strong penalty
        # Product 2: strong penalty when stock > 6
        if action[1] > 0 and self.stock[1] > 6:  
            over_order_penalty += action[1] * 5.0  # Strong penalty
            
        # Excessive ordering penalty
        excessive_order_penalty = 0
        if action[0] > 5:  # Penalize orders > 5 units
            excessive_order_penalty += (action[0] - 5) * 3.0
        if action[1] > 7:  # Penalize orders > 7 units
            excessive_order_penalty += (action[1] - 7) * 3.0
            
        # Combined reward 
        reward = base_reward - service_penalty + efficiency_bonus + ordering_efficiency - over_order_penalty - excessive_order_penalty
        self.current_day += 1
        ...
        info = {
            ...
            'service_level_bonus': efficiency_bonus,
            'shortage_penalty': service_penalty,
            'ordering_bonus': ordering_efficiency,
            'total_reward': reward
        }
